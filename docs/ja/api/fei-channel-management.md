...
!!! info "機能説明"
    ...
    AIサービスプロバイダーチャネルの完全な管理システム。チャネルのCRUD（作成、読み取り、更新、削除）、一括操作、接続性テスト、残高照会、タグ管理などの機能をサポートしています。モデル機能の同期やチャネルの複製などの高度な機能も含まれます。
...
- `group` （文字列）: グループによるフィルタリング条件
- `model` （文字列）: モデルによるフィルタリング条件
...
リクエストパラメータはありません。システムはすべての有効なチャネルの残高を自動的に更新します
...
- **パス**：`/api/channel/`
- **認証要件**：管理者
- **機能概要**：新しい AI サービスチャネルを作成します。単一、一括、マルチキーモードをサポートしています。
...
- `multi_key_mode` （文字列）: マルチキーモード。mode が "multi_to_single" の場合に必須です
...
- **パス**：`/api/channel/:id`
- **認証要件**：管理者
- **機能概要**：指定されたチャネルを物理削除します。削除後、チャネルキャッシュが更新されます。
...
- `ids` （配列）: 削除するチャネル ID のリスト。必須かつ空であってはなりません
...
- `id` （数値）: チャネル ID。URLパスを通じて渡されます
- `data` （配列）: アップストリームから取得したモデル ID のリスト
...
- **認証要件**：管理者
- **機能概要**：提供された設定情報を使用してアップストリーム API からモデルリストを取得し、新規チャネル作成時のプレビューに使用します
...
- `base_url` （文字列）: ベース URL。オプション。空の場合はデフォルト URL が使用されます
- `type` （数値）: チャネルタイプ。必須
- `key` （文字列）: APIキー。必須
- `data` （配列）: 取得されたモデルリスト
...
- `ids` （配列）: タグを設定するチャネル ID のリスト。必須かつ空であってはなりません
- `tag` （文字列）: 設定するタグ名。null を渡すとタグがクリアされます
- `data` （数値）: タグ設定に成功したチャネルの数

### タグに基づいてモデルを取得

- **インターフェース名**：タグに基づいてモデルを取得
- **HTTP メソッド**：GET
...
- `tag` （文字列）: タグ名。必須
- `data` （文字列）: このタグを持つチャネルの中でモデル数が最も多いチャネルのモデルリスト。カンマ区切り
